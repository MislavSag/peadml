...
target_variables = colnames(DT)[grepl("^ret_|bin_", colnames(DT))]
...
cols_non_features <- c("symbol", "date", "time", "right_time", "date_event",
                       ...
                       target_variables)
cols_features = setdiff(colnames(DT), c(cols_non_features, target_variables))
...
targets_new = vapply(target_variables, snakeToCamel, FUN.VALUE = character(1L), USE.NAMES = FALSE)
setnames(DT, target_variables, targets_new)
target_variables = targets_new
...
DT[, names(.SD) := lapply(.SD, as.numeric), .SDcols = bit64::is.integer64]

# remove observations with missing target
# if we want to keep as much data as possible an use only one predict horizont
# we can sleep this step
ret_targets = target_variables[grepl("^ret", target_variables)]
DT = na.omit(DT, cols = ret_targets)
